    ...
    <div className="max-w-4xl mx-auto space-y-8 p-4 min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-8">
          {/* File Selection Options - Only show if no file is selected */}
          {!file ? (
            <div className="space-y-6">
              <FileUpload
                file={file}
                onFileChange={setFile}
                label="Option 1: Upload Intraoral Photograph"
              ...
              <CameraCapture
                onCapture={(capturedFile) => {
                  console.log("Setting captured file in parent:", capturedFile.name);
                  setFile(capturedFile);
                }}
                onClear={() => setFile(null)}
                isActive={!file}
                label="Option 2: Capture Intraoral Photograph"
                required={false}
              />
            </div>
          ) : (
            /* Selected File Preview - Always show FileUpload as a "selected" indicator when file exists */
            <div className="animate-in slide-in-from-top-2 duration-300">
              <FileUpload
                file={file}
                onFileChange={setFile}
                label="Selected Photograph for Analysis"
                helperText="Review the image before running AI detection."
                required={true}
              />
            </div>
          )}
          ...
          <div className="pt-2">
            <button
              ...
              className={`w-full flex items-center justify-center gap-3 font-black py-4 px-6 rounded-xl transition-all shadow-xl text-lg ${!file || loading
                ? "bg-slate-200 text-slate-400 cursor-not-allowed"
                : "bg-blue-600 hover:bg-blue-700 text-white hover:shadow-blue-500/20 active:scale-[0.98]"
                }`}
            >
              {loading ? (
                <>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="flex-1">
              ...
              <div className="mb-6">
                ...
                <div className={`text-3xl font-bold ${result.prediction === 'Cancerous' ? 'text-red-600' : 'text-green-600'
                  }`}>
                  {result.prediction}
                </div>
